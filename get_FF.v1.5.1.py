...
Dependencies:
    pip install asyncio os re sys argparse aiohttp pathlib urllib bs4 tqdm
"""
...
async def download_file(
    ...
    async with semaphore:
        ...
        for attempt in range(1, retries + 1):
            try:
                async with session.get(url, timeout=DOWNLOAD_TIMEOUT, headers=headers) as resp:
                    resp.raise_for_status()
                    
                    with open(file_path, "wb") as f:
                        # Increased chunk size to 1MB to minimize IO overhead
...
async def extract_links(session: aiohttp.ClientSession, main_url: str, soup: BeautifulSoup = None) -> tuple[list[tuple], BeautifulSoup | None]:
    ...
    async with AsyncSpinner(f"\033[92mFound {len(extracted_links)} candidate links, scraping URLs now"):
...
def extract_text(html_content, start_marker, end_marker):
    ...
    if start_idx == -1:
        print(f"Error: Could not find '{start_marker}' in the page.")
        return
    ...
    if end_idx == -1:
        print(f"Error: Could not find the closing marker '{end_marker}' after the start marker.")
        return
