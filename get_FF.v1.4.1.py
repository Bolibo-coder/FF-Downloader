...
DEF_DLOAD_FOLDER = r"X:\FG_Dloads"
...
def save_nfo(page_soup: BeautifulSoup, download_folder: Path):
    ...
    requires_win10plus = ""
    if page_soup.find(string="Windows 10+"):
        requires_win10plus = "Game requires Windows 10+\n"
    ...
    if description_heading:
        ...
        while current and (current.name not in ['h2', 'h3', 'footer']):
            if current.name in ['p', 'ul', 'ol', 'div']:
                text = current.get_text()
                if text == 'Backwards Compatibility':
                    break
                if text.strip():
            ...
            #    print(f"- div - su-spoiler text: {spoiler_text}")
            #    if spoiler_text.strip():
            #        game_description += spoiler_text + "\n"
            #print(f"--current tag: {current.name}")
            current = current.find_next()
...
-----------------------------
    ...
    # Save NFO to file
    output_file = download_folder / "fitgirl.nfo"
